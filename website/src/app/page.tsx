...
import Client, { ObjectList, TokenPermission } from "@pelicanplatform/web-client"
					...
					<Box mt={6} mx={"auto"} width={"100%"} display={"flex"} flexDirection={"column"}>
						<Box pt={2}>
							<TextField fullWidth onChange={e => {
								setObjectPath(e.target.value)
							}} value={objectPath} id="outlined-basic" label="Object Name" variant="outlined"/>
							...
							<Button onClick={async () => {
								setObjectList(await client?.list(objectPath))
							}}>List</Button>
						</Box>
						{submitError ?
							<Box>
