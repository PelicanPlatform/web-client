...
function Page() {
    ...
    const [federations, setFederations, removeFederations] = useSessionStorage<Record<string, Federation>>(
        "federations",
        {}
    );
    const [prefixToNamespace, setPrefixToNamespace, removePrefixToNamespace] =
        useSessionStorage<ObjectPrefixToNamespaceKeyMap>("prefixToNamespace", {});
    const [codeVerifier, setCodeVerifier, removeCodeVerifier] = useSessionStorage<string | undefined>(
        "codeVerifier",
        undefined
    );
        ...
        <Box minHeight={"90vh"}>
            <Grid height={"100%"} justifyContent={"center"} container gap={2}>
                                    ...
                                    <Card key={index} sx={{ mb: 2 }} variant="outlined">
                                        <CardContent>
                                            ...
                                            >
                                                ...
                                                {obj.iscollection && (
                                                    <Button
                                                        endIcon={<MenuOpen />}
                                                        onClick={() =>
                                                            setObjectUrl(`pelican://${federationHostname}${obj.href}/`)
                                                        }
                                                    >
                                                        Explore
                                                    </Button>
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={async () => {
                                                            try {
                                                                downloadResponse(
                                                                    await get(
                                                                        `pelican://${federationHostname}${obj.href}`,
                                                                        federations[federationHostname],
                                                                        federations[federationHostname].namespaces?.[
                                                                            prefixToNamespace[objectPrefix]?.namespace
                                                                        ]
                                                                    )
                                                                );
                                                            } catch {}
                                            ...
                                            <Typography variant="body2">
                                                <strong>Type:</strong> {obj.resourcetype}
                                                {obj.iscollection ? " (Collection)" : ""}
                                            </Typography>
                                            <Typography variant="body2">
                                                <strong>Size:</strong> {obj.getcontentlength} bytes
                                            </Typography>
                                            <Typography variant="body2">
                                                <strong>Last Modified:</strong> {obj.getlastmodified}
                                            </Typography>
                                            <Typography variant="body2">
                                                <strong>Executable:</strong> {obj.executable}
                                            </Typography>
                                            <Typography variant="body2">
                ...
                <Grid size={{ xl: 7, md: 8, xs: 11 }} display={"flex"} flexDirection={"column"}>
                    ...
                    <Box mt={6} mx={"auto"} width={"100%"} display={"flex"} flexDirection={"column"}>
                        <Typography variant="h6" gutterBottom>
                            Client Federations ( for debug only )
                        </Typography>
                        <Box overflow={"auto"}>
                            <pre>
                                <code>{JSON.stringify(federations, null, 2)}</code>
                            </pre>
                        </Box>
                    </Box>
                </Grid>
